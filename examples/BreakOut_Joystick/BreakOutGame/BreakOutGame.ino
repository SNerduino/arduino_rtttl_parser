...
int leftRightAnalogPin = 3;     // potentiometer wiper (middle terminal) connected to analog pin 3
int upDownAnalogPin = 4;     // potentiometer wiper (middle terminal) connected to analog pin 3
int jsBTNPin = 2;
int btnPin = 6;
// ===================================================================================
// Melodies
...
int lives =5;
bool isJSXPressed=false;
bool isJSYPressed=false;
bool isJSBTNPressed=false;
long debounceTime=0;
void reset()
{
    lives=5;
...
void loop() {
  ...
  if(!playingMelody)
  {
    ...
    int x = analogRead(leftRightAnalogPin);
    int y = analogRead(upDownAnalogPin);
    int jsBTNVal = analogRead(jsBTNPin);
    ...
    Serial.print(str);
    Serial.print(", jsbtn = ");
    itoa(jsBTNVal,str,10);
    Serial.print(str);
    Serial.print(", btn = ");
    itoa(btnVal,str,10);
    Serial.print(str);
    Serial.print(", isPlaying = ");
    itoa(isPlaying,str,10);
    Serial.print(str);
    Serial.println("");

    // Detect right 
    if(x<50 && !isJSXPressed)
    {
      if(racketPos<6)
      {
         racketPos++;
    ...
    // Detect up restart game
    if(!btnVal && time_-debounceTime>100)
    {
      debounceTime=time_;
      reset();
    }
